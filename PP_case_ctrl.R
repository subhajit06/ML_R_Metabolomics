...
df_sample_data_PP = df_sample_data_PP_sorted[, c(7,seq(11,312))]

set.seed(101)

cv_5 = trainControl(method = "cv", number = 10)

PP_case_ctrl_elnet = train(
  class ~ ., data = df_sample_data_PP,
  method = "glmnet",
  trControl = cv_5,
  tuneLength = 10
)

br_el = get_best_result(PP_case_ctrl_elnet)

X = model.matrix(class ~ ., df_sample_data_PP)[, -1]
y = df_sample_data_PP$class
...
PP_case_ctrl_compund_name_list = df_compound[id_list_1, 2]

saveRDS(id_list_1,"./id_list_PP_case_ctrl.Rds")


df_plot = df_sample_data_PP[,id_list_1]
df_plot$type = df_sample_data_PP$class
df_plot$sample_id = df_sample_data_PP_sorted$sample_id
